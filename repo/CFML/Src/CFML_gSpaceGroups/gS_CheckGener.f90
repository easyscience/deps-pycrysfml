...
SubModule (CFML_gSpaceGroups) gS_CheckGener
   ...
   Module Subroutine Check_Gener(Gen_in, Gen_out, ngen)
      ...
      !---- Local variables ----!
      logical                                          :: init, newgen
      integer                                          :: i,j,d,d_in,ngen_in,nop,nop_in,invt,ier
      character(len=80), dimension(size(gen_in))       :: genAux
      type(Symm_Oper_Type), dimension(0:512)           :: Op
      type(rational),       dimension(:,:),allocatable :: Mat
      character(len=80)                :: aux_str
      ...
      do i=1, ngen_in
         ...
         call Get_Mat_From_Symb_Int(gen_in(i), Mat, invt)
      ...
      if (ngen > 0) then
         ...
         !Assure that the positive time reversal contains the '+' symbol
         do i=1,ngen
           j=index(gen_out(i),",",back=.true.)
           read(gen_out(i)(j+1:),*,iostat=ier) d
           if(ier /= 0) cycle
           if(d > 0) gen_out(i)(j+1:)="+1"
         end do
      end if
