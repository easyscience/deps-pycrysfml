...
SubModule (CFML_IOForm) Format_CIF
   ...
   Module Subroutine Read_CIF_Atoms(cif, Atm, i_ini, i_end)
      ...
      type(atList_type),  intent(out) :: Atm  !List
      ...
      do i=j_ini, j_end
         ...
         else  !If not provided in the atom loop assume that we are dealing with isotropic U's
            atm%atom(n)%utype="U_ij"
            atm%atom(n)%thtype='iso'
         end if
      end do
...
!      if (n <= 0) return
!      select type (at => atm%atom)
!         ! Los call allocate_atom_list(0 están comentados porque
!         ! provocan un crash con gfortran cuando se hace la interfaz
!         ! con python. Sin embargo, si hacemos el deallocate fuera
!         ! del select type, funciona.
!         type is (atm_type)
...
!      deallocate(atm%active)
   ...
   Module Subroutine Read_CIF_AtomSite_Displace_Fourier(cif, AtmList,i_ini,i_end)
      ...
      if (cif%nlines <=0) then
         err_CFML%Ierr=1
         err_CFML%Msg="Read_MCIF_AtomSite_Displace_Fourier: 0 lines "
         return
      end if
      ...
      !> Search loop
      found=.false.
      str="_atom_site_displace_Fourier"
      nl=len_trim(str)
      ...
      do i=j_ini,j_end
         ...
         select case (trim(line))
            case ('_atom_site_displace_Fourier.atom_site_label')
               j=j+1
               lugar(1)=j
            case ('_atom_site_displace_Fourier.axis')
               j=j+1
               lugar(2)=j
            case ('_atom_site_displace_Fourier.id')
               j=j+1
               lugar(3)=j
            case ('_atom_site_displace_Fourier.wave_vector_seq_id')
               j=j+1
               lugar(4)=j
            case ('_atom_site_displace_Fourier_param.id')
               j=j+1
               lugar(5)=j
            case ('_atom_site_displace_Fourier_param.cos')
               j=j+1
               lugar(6)=j
            case ('_atom_site_displace_Fourier_param.sin')
               j=j+1
               lugar(7)=j
            case ('_atom_site_displace_Fourier_param.modulus')
               j=j+1
               lugar(8)=j
            case ('_atom_site_displace_Fourier_param.phase')
               j=j+1
               lugar(9)=j
            case ('_atom_site_displace_Fourier_param.cos_symmform')
               j=j+1
               lugar(10)=j
            case ('_atom_site_displace_Fourier_param.sin_symmform')
               j=j+1
               lugar(11)=j
            case ('_atom_site_displace_Fourier_param.modulus_symmform')
               j=j+1
               lugar(12)=j
            case ('_atom_site_displace_Fourier_param.phase_symmform')
               j=j+1
               lugar(13)=j
         end select
      end do
      ...
      if (np == 0) then
         err_CFML%Ierr=1
         err_CFML%Msg="Read_CIF_AtomSite_Displace_Fourier: Check the loop! "
         return
      end if
      ...
      if (lugar(1) == 0) then
         err_CFML%Ierr=1
         err_CFML%Msg="Read_CIF_AtomSite_Displace_Fourier: No atom label was identified! "
         return
      end if

      if (lugar(2) == 0) then
         err_CFML%Ierr=1
         err_CFML%Msg="Read_CIF_AtomSite_Displace_Fourier: Coordinate system No specified!"
         return
      end if

      if (lugar(4) == 0) then
         err_CFML%Ierr=1
         err_CFML%Msg="Read_CIF_AtomSite_Displace_Fourier: Identification for wave vector no specified!"
         return
      end if
      ...
      if (lugar(6) > 0) then
         if (lugar(7) == 0 .or. lugar(8) > 0 .or. lugar(9) > 0) then
            err_CFML%Ierr=1
            err_CFML%Msg="Read_CIF_AtomSite_Displace_Fourier: Cosine/Sine components of the displace modulation not provided!"
            return
         end if
      end if
      ...
      if (lugar(9) > 0) then
         if (lugar(8) == 0 .or. lugar(6) > 0 .or. lugar(7) > 0) then
            err_CFML%Ierr=1
            err_CFML%Msg="Read_CIF_AtomSite_Displace_Fourier: Modulus/phase components of the displace modulation not provided!"
            return
         end if
      end if
      ...
      do i=j_ini, j_end
         ...
         if (ic == 0) then
            err_CFML%Ierr=1
            err_CFML%Msg="Read_CIF_AtomSite_Displace_Fourier: Revise the line: "//trim(line)
            return
         end if
         ...
         do k=1,Atmlist%natoms
            ...
            if (iv /= 1) then
               err_CFML%Ierr=1
               err_CFML%Msg="Read_CIF_AtomSite_Displace_Fourier: ID for Wave vector identification!"
               return
            end if
            ...
            if (lugar(6) > 0) then
               ...
               if (iv /= 1) then
                  err_CFML%Ierr=1
                  err_CFML%Msg="Read_CIF_AtomSite_Displace_Fourier: Error in cosine component!"
                  return
               end if
               xv(1)=vet1(1)
               xv_std(1)=vet2(1)
               ...
               if (iv /=1) then
                  err_CFML%Ierr=1
                  err_CFML%Msg="Read_CIF_AtomSite_Displace_Fourier: Error in sine component!"
                  return
               end if
               xv(2)=vet1(1)
               xv_std(2)=vet2(1)
               ...
               select case (trim(dire(lugar(2))))
                  ...
                  case default
                     err_CFML%Ierr=1
                     err_CFML%Msg="Read_CIF_AtomSite_Displace_Fourier: Axis system is not defined!"
                     return
               end select
               ...
               if (iv /=1) then
                  err_CFML%Ierr=1
                  err_CFML%Msg="Read_CIF_AtomSite_Displace_Fourier: Error in modulus component!"
                  return
               end if
               xv(1)=vet1(1)
               xv_std(1)=vet2(1)
               ...
               if (iv /=1) then
                  err_CFML%Ierr=1
                  err_CFML%Msg="Read_CIF_AtomSite_Displace_Fourier: Error in phase component!"
                  return
               end if
               xv(2)=vet1(1)
               xv_std(2)=vet2(1)
               ...
               err_CFML%Ierr=1
               err_CFML%Msg="Read_CIF_AtomSite_Displace_Fourier: Error in components of Modulation vector!"
               return
            end if
            ...
            select type(At => Atmlist%atom)
               class is (ModAtm_Std_Type)
                  At(k)%Dcs(i1,nq)=xv(1)
                  At(k)%Dcs(i2,nq)=xv(2)
                  At(k)%Dcs_std(i1,nq)=xv_std(1)
                  At(k)%Dcs_std(i2,nq)=xv_std(2)
   ...
   Module Subroutine Read_XTal_CIF(cif, Cell, Spg, AtmList, Nphase, database_path)
      ...
      if (it > 0) then
        ...
        if (present(database_path)) then
            call set_spacegroup(spp,Spg,database_path=database_path)
        else
      ...
      if (len_trim(spp) <= 0) then
         ...
         if(N_Oper > 0) then
            if (present(database_path)) then
               call set_spacegroup(spp, SpG, N_Oper, Oper_Symm(1:N_Oper),database_path=database_path)
            else
   ...
   End Subroutine Write_CIF_Spg
    ...
    !!----    Traslation applied. Intended for writing CIF files.
    !!--<<        2_555     : N_Op = 2, Tr=( 0.0, 0.0, 0.0)
    !!----        3_456     : N_Op = 3, Tr=(-1.0, 0.0, 1.0)
    !!-->>
    !!----
    !!---- Update: October - 2024 (RJA)
    !!
    Pure Module Function Write_CIF_SymTrans(N,Tr) Result(Code)
       !---- Arguments ----!
       integer,                       intent(in)  :: N
