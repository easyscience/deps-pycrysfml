...
Module  TOF_Diffraction
 ...
 Contains
   ...
   Subroutine Sum_Jorgensen_Vondreele_Peaks(i,Tof,Ycalc,Vsa,CalDer)
      ...
      !alfa=alfa0 + alfa1*sqv + alfaq*sqvr + alfi(nn,n_pat) + d_alfa  !doesn't work
      !beta=beta0 + beta1/d4  + betaq/dd2  + beti(nn,n_pat) + d_beta
      !refs(nn,2,n_pat)=dtt1(n_pat)*d+shv + d_pos + sl_i(nn,n_pat)          !Update the reflection positions
      !POWGEN:   alpha1,Beta0,Beta1,BetaQ
      !Position:
      ! TOF = Dtt1*d + Dtt2*d2 +  ZERO +  DIFB/d
      ! Alpha = Alpha1/d = alpha1*sqv
      ! Beta=Beta0 + Beta1/d^4 + BetaQ/d^2
      ! sqvr=1/sqrt(d)      !
      !  alfa=alfa0 + alfa1*sqv + alfaq*sqvr + alfi(nn,n_pat) !+ d_alfa  
      !  beta=beta0 + beta1/d4  + betaq/dd2  + beti(nn,n_pat) !+ d_beta

      !sigma=( (sig2+gsiz)*dd2+ (sig1+d_sig1+gstr)+ dst*(1.0-par(iph,33,n_pat))**2 )*dd2 + sig0 + sigq*d  + d_hg + blayer*blayer/5.54517744448
      !gamma=gamma0+(gamma1+strl+par(iph,33,n_pat)*hss(nm))*d+(gamma2+lsize+dsiz+d_gam2)*dd2 + d_hl
